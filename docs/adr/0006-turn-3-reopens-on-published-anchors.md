...
> Amends **ADR-0005** on the finals rule and on promotion, and reverses the "no turn 3" decision recorded when the turn-2 map (#22) closed.
...
1. **The Maldonado TE is measured as one stacked candidate, not a chain.** Its published parts — TE prior weight 20 → 1 (+0.00025), `//100` and `//1000` keys (+0.00012), exact and floored commute keys — each fall below the +0.0003 promotion rule, so a single-field chain would kill every one by construction. Lowering the gate is what ADR-0005 forbids; stacking them is how turn 1 already handled its survivors. Two configurations are declared (prior 1 and prior 5), and the Axis dies only if both fail. No ablation if it wins: the stack enters whole.
...
4. **`hpsearch_lightgbm_best_confirm` is promoted by recorded override.** Its canonical-seed Paired Delta is +0.00029, 0.00001 under the rule; its Confirmation Run held the sign on 15 of 15 folds across three seeds. The override is a new, explicit field written by the one ledger writer — never a silent threshold change — and it is the driving dev's to run, like every promotion (ADR-0003).
