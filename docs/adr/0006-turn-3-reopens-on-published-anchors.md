...
5. **The finals rule gains a Proven Final clause.** ADR-0005's "best CV + best single-family model" stands, but at least one final must be a **Proven Final**. If the best CV depends on turn-3 code, the second final is `hpsearch_lightgbm_best_confirm`. ADR-0005 moved the suspect from the Floor to the Blend; the suspect now is any code path that has never produced a scored submission, and a new TE that leaks would sink a Blend and its own single-family model together. As implemented (#38): the second slot is the best-CV single-family model that is Proven — its family and Frame configuration already produced a scored submission — so at least one final always is; `hpsearch_lightgbm_best_confirm` takes the second slot only if no other confirmed model is Proven. Both slots require a passing Confirmation Run. Replayed on the #32 ledgers, this reproduces that turn's finals (`hpsearch_lightgbm_best_confirm` + `income_te_tuned`).
